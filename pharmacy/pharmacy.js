  ...
  filteredInventory.forEach(med => {
    ...
    const cardHTML = `
      <div class="med-card animate-card" data-id="${med.id}">
        ...
        <button class="add-to-cart-btn">INITIALIZE ACQUISITION</button>
      </div>
    `;
...
function initializeMedicalCardTilt() {
  ...
  const interactivePharmacyElements = document.querySelectorAll('.add-to-cart-btn, .filter-btn');
  interactivePharmacyElements.forEach(el => {
    el.addEventListener('mouseenter', () => {
      const cursorElement = document.getElementById('customCursor');
      if (cursorElement) cursorElement.classList.add('hovering');
    });
    el.addEventListener('mouseleave', () => {
      const cursorElement = document.getElementById('customCursor');
      if (cursorElement) cursorElement.classList.remove('hovering');
    });
  });
}
